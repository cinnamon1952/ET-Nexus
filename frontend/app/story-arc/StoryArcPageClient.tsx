...
import { Persona, useAppContext } from "@/lib/context";
import {
  Loader2,
  TrendingUp,
  TrendingDown,
...
export default function StoryArcPageClient({ initialArticleId, initialPersona }: Props) {
  ...
  }, [initialPersona, persona, setPersona]);

  const loadArc = useCallback(async (arcId: string) => {
    setArcLoading(true);
    try {
      const arc = await fetchStoryArc(arcId, persona);
      setSelectedArc(arc);
      router.replace(`/story-arc?articleId=${arcId}&persona=${persona}`);
    } catch {
      setSelectedArc(null);
    } finally {
      setArcLoading(false);
  ...
  useEffect(() => {
    ...
    async function loadArcSummaries() {
      setLoading(true);
      try {
        const data = await fetchStoryArcs(persona);
        if (cancelled) return;
        setArcs(data.arcs);

        const defaultArcId = initialArticleId || data.arcs[0]?.id;
        if (!defaultArcId) return;
        await loadArc(defaultArcId);
      } catch {
        if (cancelled) return;
        if (initialArticleId) {
          await loadArc(initialArticleId);
        }
      } finally {
    ...
    void loadArcSummaries();

    return () => {
      cancelled = true;
    };
  }, [persona, initialArticleId, loadArc]);
        ...
        <div className="relative flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-[var(--color-muted)]">
              Article-linked timelines with related developments, key players, sentiment tracking, and AI predictions.
            </p>
          </div>
        </div>
      </header>
                ...
                <div className="rounded-xl bg-[var(--color-card)] border border-[var(--color-border)] p-6">
                  <div className="flex items-end gap-2 h-32">
                    {selectedArc.sentiment_trajectory.map((point, i) => {
                      const normalized = (point.score + 1) / 2;
